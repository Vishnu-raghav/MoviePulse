  ...
  return (
    <div className="p-4 sm:p-6 text-white bg-gray-900 min-h-screen">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row items-center gap-6 bg-gray-800 p-4 sm:p-6 rounded-lg shadow-lg">
          <img
            src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`}
            alt={movie.title}
            className="w-full sm:w-80 md:w-96 rounded-lg shadow-md"
          />
          ...
          {trailer ? (
            <div className="flex-1 w-full">
              ...
              <iframe
                className="w-full h-48 sm:h-64 md:h-80 rounded-lg shadow-md"
              ...
              cast.map((actor) => (
                <div key={actor.id} className="text-center">
                  <img
                    ...
                    alt={actor.name}
                    className="rounded-lg w-24 h-32 sm:w-28 sm:h-36 object-cover mx-auto shadow-md"
                  />
                  <p className="text-xs sm:text-sm mt-2">{actor.name}</p>
                </div>
              ))
            ) : (
